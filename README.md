...
```bash
git-viz .                                   # 分析当前目录仓库，生成 ./git-viz-report.html
git-viz /path/to/repo -o report.html
git-viz . --since 2026-01-01 --until 2026-07-01
git-viz . --author "Wei,someone@example.com"
git-viz . --exclude "node_modules/**,dist/**"
git-viz . --branch main
git-viz . --all-branches
git-viz . --max-commits 5000
git-viz . --open
git-viz . --format json --output data.json
```
...
本期未实现：增量分析缓存、`.git-viz.yaml` 配置文件、CI 定时报告、完整分支拓扑图谱
